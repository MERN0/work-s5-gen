...
from artifacts.sys5.graph.build import build_graph
from artifacts.sys5.llm.client import build_chat_model
...
def run_pipeline(config: Sys5Config) -> PipelineResult:
    llm = build_chat_model(config)
    graph = build_graph(config.agent_chain)
...
def _recursion_limit(config: Sys5Config) -> int:
