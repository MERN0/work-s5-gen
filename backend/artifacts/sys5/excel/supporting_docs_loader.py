...
patterns — names and shapes vary per project). No column names are ever
hardcoded here: shape genuinely differs project to project, so the only thing
extracted generically is "every non-empty row's values," left for
matching/fuzzy_context.py to search over.

Deliberately includes every OTHER tab of the requirements workbook itself
(e.g. a "Communication Matrix Sheet" or "Application Parameters" tab living
alongside "System Requirements" in the same file) — that's a real observed
project layout, not just separate files.
"""
...
from artifacts.sys5.excel.raw_grid import RawSheet, find_header_row, load_workbook_sheets, row_to_dict
...
def load_supporting_sheets(config: Sys5Config) -> list[RawSheet]:
...
def _candidate_files(config: Sys5Config) -> list[Path]:
