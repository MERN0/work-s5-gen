...
from artifacts.sys5.models.requirement import EnrichedRequirement, FeatureMeta, Requirement
from artifacts.sys5.models.test_case import ActionStep, TestCase, VerificationStep
from artifacts.sys5.output.workbook_builder import build_workbook


def _config(tmp_path) -> Sys5Config:
    out = tmp_path / "out"
    out.mkdir()
    return Sys5Config(output_dir=out, input_folder=tmp_path, req_filename="", req_sheet_name="005")
...
def _state(tmp_path):
    requirement = Requirement(
        ...
        matched_keyword="sys qualification test",
...
def test_build_workbook_produces_expected_sheets_and_content(tmp_path):
    ...
    assert row_dict["TestCase ID"] == "TMHC_SQMTC_001"
    ...
    assert items_row[3] == "TMHC_SQMTC_001"
