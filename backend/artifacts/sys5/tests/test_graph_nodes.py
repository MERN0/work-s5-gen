...
import artifacts.sys5.graph.nodes as nodes_module
...
_ALL_AGENTS = ["planning_agent", "generation_agent", "verification_agent", "qa_agent"]


def _config(**overrides) -> Sys5Config:
    defaults = dict(
        ...
        agent_chain=[AgentStep(index=i, agent_name=name) for i, name in enumerate(_ALL_AGENTS)],
    )
...
def test_finalize_item_assigns_id_and_shrinks_queue():
    ...
    assert update["results"][0].testcase_id == "TMHC_DEMO_SQMTC_001"
