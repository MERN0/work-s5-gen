...
from artifacts.sys5.prompts import planning_agent
from artifacts.sys5.prompts.registry import build_messages, is_agent_selected, resolve_system_prompt
...
def test_resolve_system_prompt_appends_known_domain_to_default():
    ...
    assert "Advanced Driver Assistance Systems (ADAS)" in prompt
...
def test_resolve_system_prompt_appends_known_domain_to_agent_chain_override():
    ...
    assert "Body Control Module (BCM)" in prompt
