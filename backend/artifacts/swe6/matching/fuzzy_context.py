...
from artifacts.swe6.excel.supporting_docs_loader import SupportingEntity
from artifacts.swe6.models.requirement import (
    EnrichedRequirement,
    Requirement,
...
def enrich_requirement(
    requirement: Requirement, entities: list[SupportingEntity], config: Swe6Config
) -> EnrichedRequirement:
    if not entities:
    ...
    logger.debug(
        "[swe6] Fuzzy match for %s: tier1(row, token_set_ratio>=%d)=%d hit(s), "
