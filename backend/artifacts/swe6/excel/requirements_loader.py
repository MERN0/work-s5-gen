"""Turns the raw SWE.1 software requirements workbook into Requirement records.

Algorithm, per target sheet:
1. Scan every cell of every row for a config.swe6_keywords hit (substring
...
from artifacts.swe6.excel.raw_grid import (
    MAX_HEADER_CELL_LEN,
    RawSheet,
    find_header_row,
...
from artifacts.swe6.exceptions import RequirementsFileError
from artifacts.swe6.models.requirement import FeatureMeta, Requirement

logger = logging.getLogger(__name__)

_REQ_TEXT_HEADER_HINTS = ["requirement text", "software requirement", "system requirement", "description", "requirement"]
...
def load_requirements(config: Swe6Config) -> list[Requirement]:
    ...
    logger.info("[swe6] Scanning %d sheet(s) of %s for requirement rows: %s", len(sheets), path.name, [s.name for s in sheets])

    requirements: list[Requirement] = []
    for sheet in sheets:
        requirements.extend(_extract_requirements_from_sheet(sheet, config))
    logger.info("[swe6] Extracted %d requirement row(s) total from %s.", len(requirements), path.name)
    return requirements


def _extract_requirements_from_sheet(sheet: RawSheet, config: Swe6Config) -> list[Requirement]:
    matched_rows = _scan_for_keyword_rows(sheet, config)
    if not matched_rows:
        logger.info("[swe6] Sheet %r: no keyword-matched rows.", sheet.name)
        return []

    header_row_index, header = find_header_row(sheet, anchor_row=matched_rows[0][0])
    logger.info(
        "[swe6] Sheet %r: %d row(s) matched a swe6 keyword; header row=%s.",
    ...
    for row_index, matched_keyword in matched_rows:
        ...
        logger.info(
            "[swe6] Sheet %r row %d: req_id=%r (matched keyword %r)", sheet.name, row_index, req_id, matched_keyword,
        )
        requirements.append(
            Requirement(
                req_id=req_id,
...
def _scan_for_keyword_rows(sheet: RawSheet, config: Swe6Config) -> list[tuple[int, str]]:
...
def _row_matches_keyword(row_values: list, config: Swe6Config) -> str | None:
    for cell_value in row_values:
        text = _normalize(cell_value)
        if not text:
            continue
        for keyword in config.swe6_keywords:
