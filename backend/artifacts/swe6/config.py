"""Swe6Config: the typed, defaulted config the rest of the pipeline works
against, built from the raw dict runner.py's generate_swe6() assembles from a GenerationRequest.

New swe6-specific keys (keyword lists, attempt limits, ID scheme, etc.) all
have defaults, so the example config in the project brief works unmodified —
a project only needs to override the ones it cares about.
"""
from __future__ import annotations

from pathlib import Path
from typing import Any, Literal

from pydantic import BaseModel, Field

from artifacts.common.schema import AgentStep
from artifacts.swe6.exceptions import Swe6ConfigError

# Mirrors sys5's DEFAULT_SYS5_KEYWORDS but scans for *software* qualification
# test rows (SWE.6), not system-level ones — a SYS2 sheet using "sys qt" and a
# SWE.1 sheet using "sw qt" for the same project would otherwise collide if
# this pipeline reused sys5's keyword list.
DEFAULT_SWE6_KEYWORDS: list[str] = [
    "swe6 test",
    "swe6",
    "sw test",
    "sw qt",
    "sw qualification test",
    "software qualification",
    "software qualification test",
    "swqtc",
]

# Raw dict keys already handled explicitly in from_raw() below — excluded from
# the generic passthrough so we never pass the same field twice to Swe6Config(...).
_RAW_KEYS_HANDLED_EXPLICITLY = {
    "project_name", "username", "version", "language", "domain", "artifact",
    "input_format", "output_format", "model", "output_dir", "output_folder_path",
    "input_folder_path", "input_folder", "uploaded_files", "agent_chain",
    "req_filename", "req_sheet_name",
}


class Swe6Config(BaseModel):
    model_config = {"arbitrary_types_allowed": True}

    # --- standard fields (mirrors what runner.py's generate_swe6() already extracts from GenerationRequest) ---
    project_name: str = "default_project"
    username: str = "default_user"
    version: str = "v1.0"
    language: str = ""
    domain: str = ""
    artifact: str = ""
    input_format: str = ""
    output_format: str = "excel"
    model: str = ""
    output_dir: Path
    input_folder: Path
    uploaded_files: list[str] = Field(default_factory=list)
    agent_chain: list[AgentStep] = Field(default_factory=list)
    req_filename: str = ""
    req_sheet_name: str = ""  # "" means: process every sheet in the workbook

    # --- swe6-specific knobs, all defaulted ---
    swe6_keywords: list[str] = Field(default_factory=lambda: list(DEFAULT_SWE6_KEYWORDS))
    keyword_match_threshold: int = 85
    fuzzy_match_threshold: int = 60
    token_match_threshold: int = 85
    max_supporting_context_items: int = 25
    max_validation_attempts: int = 2
    max_correction_attempts: int = 1
    fallback_priority: str = "P3"
    fallback_mode_of_execution: str = "Manual"
    format_profile: str = "default"
    test_case_id_pattern: str = "{project_code}_SWQTC_{n:03d}"
    project_code: str | None = None
    llm_temperature: float = 0.1
    structured_output_method: Literal["json_schema", "function_calling"] = "json_schema"
    intermediate_dir: Path | None = None

    def resolved_project_code(self) -> str:
        return self.project_code or self.project_name.upper()

    def active_agent_names(self) -> set[str]:
        """Which agent_name values the caller's agent_chain actually selected
        - the graph/nodes layer uses this to decide whether an agentic step
        (planning/verification/correction) runs at all, not just whether its
        prompt is overridden.
        """
        return {step.agent_name for step in self.agent_chain}

    def agent_max_retries(self, agent_name: str, default: int) -> int:
        """The retry/attempt budget for one agent: that agent's own
        AgentStep.max_retries if it's present in agent_chain, else the given
        pipeline-wide default (e.g. max_validation_attempts/
        max_correction_attempts) - see graph/routing.py.
        """
        for step in self.agent_chain:
            if step.agent_name == agent_name:
                return step.max_retries
        return default

    @classmethod
    def from_raw(cls, config: dict[str, Any]) -> "Swe6Config":
        output_dir = config.get("output_folder_path") or config.get("output_dir", ".")
        input_folder = config.get("input_folder_path", "")

        extra = {
            k: v
            for k, v in config.items()
            if k in cls.model_fields and k not in _RAW_KEYS_HANDLED_EXPLICITLY
        }

        try:
            return cls(
                project_name=config.get("project_name", "default_project"),
                username=config.get("username", "default_user"),
                version=config.get("version", "v1.0"),
                language=config.get("language", ""),
                domain=config.get("domain", ""),
                artifact=config.get("artifact", ""),
                input_format=config.get("input_format", ""),
                output_format=config.get("output_format", "excel"),
                model=config.get("model", ""),
                output_dir=Path(output_dir),
                input_folder=Path(input_folder) if input_folder else Path("."),
                uploaded_files=config.get("uploaded_files", []) or [],
                # AgentStep.model_validate handles a dict, an AgentStep instance, or
                # any other object exposing the same attributes (e.g. a caller's own
                # AgentStep-shaped pydantic model) via from_attributes - see AgentStep.
                agent_chain=[AgentStep.model_validate(a) for a in (config.get("agent_chain") or [])],
                req_filename=config.get("req_filename", ""),
                req_sheet_name=config.get("req_sheet_name", ""),
                **extra,
            )
        except Swe6ConfigError:
            raise
        except Exception as exc:
            raise Swe6ConfigError(f"Could not build Swe6Config from raw config: {exc}") from exc
