...
from app.core.artifacts.swe6.excel.raw_grid import RawSheet, find_header_row, row_to_dict
from app.core.artifacts.swe6.formats.base import ColumnSpec, FormatProfile, WorkbookBuildContext
from app.core.artifacts.swe6.models.requirement import FeatureMeta
...
def build_cover_page(ctx: WorkbookBuildContext) -> dict:
    ...
    return {
        ...
        "doc_meta": {
            "Document No": f"TD/{config.project_name}/{config.artifact or 'SWE6'}",
        ...
        "revision_rows": [
            {
                ...
                "Change Description": f"Auto-generated SWE6 test cases for {title}",
                "Date": datetime.now().strftime("%Y-%m-%d"),
                "Created by": config.username,
                "Approved by": config.username,
                "Description": f"{config.artifact or 'SWE6'} generation run",
            }
        ],
    }
