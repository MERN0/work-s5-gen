...
from app.core.artifacts.swe6.exceptions import LlmClientError

# Picks up OPENAI_API_KEY / SWE6_LLM_API_BASE from a local .env file (see
...
def build_chat_model(config: Swe6Config) -> ChatOpenAI:
    api_key = os.environ.get("OPENAI_API_KEY")
    if not api_key:
        raise LlmClientError("OPENAI_API_KEY is not set in the environment.")
    api_base = os.environ.get("SWE6_LLM_API_BASE", DEFAULT_API_BASE)

    logger.info("[swe6] Building LLM client: model=%r api_base=%r temperature=%s", config.model, api_base, config.llm_temperature)
    try:
        return ChatOpenAI(
            model=config.model,
...
def call_structured(
    ...
    """
    logger.debug(
        "[swe6] LLM request: output_model=%s method=%s (%d chars of user content)",
    ...
    logger.debug("[swe6] LLM response received for output_model=%s", output_model.__name__)
    return result
