...
EXAMPLE_CONFIG = {
    ...
    "req_filename": "TE_TMHC_HILS Development & Testing_Software Requirements.xlsx",
...
def test_from_raw_parses_the_example_config_unmodified():
    config = Swe6Config.from_raw(EXAMPLE_CONFIG)
    ...
    # swe6-specific defaults are present even though the raw config never mentions them
    assert config.max_validation_attempts == 2
    assert config.max_correction_attempts == 1
    assert "software qualification" in config.swe6_keywords


def test_from_raw_honors_overrides_for_new_keys():
    raw = dict(EXAMPLE_CONFIG)
    raw["max_correction_attempts"] = 3
    raw["swe6_keywords"] = ["custom keyword"]

    config = Swe6Config.from_raw(raw)

    assert config.max_correction_attempts == 3
    assert config.swe6_keywords == ["custom keyword"]
