...
"SW Qualification Test" keyword alongside a "Precondition:/Test Procedure:"
authored block - the real shape observed in the project's own Software
...
def requirements_workbook(tmp_path):
    ...
    ws.append(
        [
            2,
            "When switch_input_flag is set (1), the Light_Control module shall set "
            "light_output_state to LIGHT_ON within one control cycle.",
            "Functional Requirement",
            "SW Qualification Test",
            "Precondition:\n1. Module initialized\n2. No faults present\n\n"
            "Test Procedure:\n1. Set switch_input_flag via the diagnostic API",
            "Approved",
        ]
    )
...
def _config(tmp_path, req_filename: str, **overrides) -> Swe6Config:
    defaults = dict(
...
def test_load_requirements_finds_only_the_keyword_matched_row(tmp_path, requirements_workbook):
    ...
    assert "Light_Control module shall set" in requirements[0].requirement_text
