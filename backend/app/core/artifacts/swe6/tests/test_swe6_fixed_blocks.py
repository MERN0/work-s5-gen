"""Regression tests for the two "do not change" blocks in swe6.py: the raw
config -> field extraction (now feeding Swe6Config.from_raw) and the
...
from app.core.artifacts.swe6 import swe6
...
from app.core.artifacts.swe6.pipeline import PipelineResult
...
def test_generate_extracts_fields_and_zips_only_non_zip_output_dir_files(tmp_path, monkeypatch):
    ...
    def fake_run_pipeline(config: Swe6Config) -> PipelineResult:
        captured["config"] = config
        xlsx_path = output_dir / "swe6_005_20260101_000000.xlsx"
    ...
    monkeypatch.setattr(swe6, "run_pipeline", fake_run_pipeline)

    zip_path = swe6.generate(_raw_config(output_dir, input_dir))

    # Top block: raw dict fields must have reached Swe6Config unchanged.
    ...
    with zipfile.ZipFile(zip_path) as zf:
        names = zf.namelist()
    assert "swe6_005_20260101_000000.xlsx" in names
    assert not any(name.endswith(".zip") for name in names)


def test_generate_reraises_swe6_errors_without_producing_a_zip(tmp_path, monkeypatch):
    output_dir = tmp_path / "out"
    input_dir = tmp_path / "in"
    input_dir.mkdir()

    def failing_run_pipeline(config: Swe6Config):
        raise RequirementsFileError("missing file")

    monkeypatch.setattr(swe6, "run_pipeline", failing_run_pipeline)

    with pytest.raises(RequirementsFileError):
        swe6.generate(_raw_config(output_dir, input_dir))
