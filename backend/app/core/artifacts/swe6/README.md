...
(returns a zip path). See `swe6.py` for the two blocks marked "do not
...
because `swe6.py`'s fixed "Step 2" block zips *everything* it finds there.
...
`python -m app.core.artifacts.swe6.swe6` runs the whole `generate()` flow
...
| `req_id` traceability fix, structured logging, `.env.example`, offline dry run (`__main__` in `swe6.py`) | Already present (added in an earlier pass) | Built in from the start, so swe6 launches at parity with sys5's current state rather than sys5's original bare-bones version |
