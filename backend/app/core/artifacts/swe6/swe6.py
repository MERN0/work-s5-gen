...
from app.core.artifacts.swe6.exceptions import Swe6Error
from app.core.artifacts.swe6.pipeline import run_pipeline

logger = logging.getLogger(__name__)

def generate(config: dict) -> str:
    """
    SWE6 (Software Qualification Test Cases) generator.

    Reads SWE.1 software requirements plus project-specific supporting docs
    (function/API lists, interface specs, error/return-code tables,
    calibration parameters, test patterns) from input_folder_path, and
    generates a formatted SWE6 test case workbook via a LangGraph pipeline
    (per-requirement planning, generation, validation, and correction).
    Returns the output zip path. Accepts the standardized SME6-compatible
    config payload; agent_chain prompt overrides are honored per agent (see
    prompts/registry.py).
    """
...
######
    ...
    logger.info(
        f"[swe6] generate() called: project={project_name}, username={username}, version={version}, "
    ...
    if agent_chain:
        ...
        logger.info(
            f"[swe6] Received agent_chain with {len(agent_chain)} agent(s): {agent_names} "
            f"(prompts NOT extracted - dummy generator),"
        )
    else:
        logger.info("[swe6] No agent_chain provided.")

    os.makedirs(output_dir, exist_ok=True)

    # Step 1: run the real SWE6 pipeline. It builds a validated Swe6Config
    ...
    try:
        swe6_config = Swe6Config.from_raw(config)
...
#####Donot change this part in code
    ...
    zip_path = os.path.join(output_dir, f"swe6_{project_name}_{timestamp}.zip")
...
if __name__ == "__main__":
    ...
    #
    # Run with: python -m app.core.artifacts.swe6.swe6
    import shutil
    import tempfile
    ...
    from app.core.artifacts.swe6.logging_config import configure_logging
    from app.core.artifacts.swe6.paths import DEFAULT_RUNS_DIR
    ...
    def _fake_call_structured(llm, output_model, system_prompt, user_content, structured_output_method="json_schema"):
        if output_model is TestAspectPlan:
            return TestAspectPlan(
                aspects=[TestAspect(aspect_id="A1", title="Nominal input flag handling", rationale="dry run")]
            )
        if output_model is GeneratedTestCase:
            return GeneratedTestCase(
                feature="Dummy Feature",
                requirement_ids=["1"],
                test_type="Normal_software",
                objective="Verify the function returns light-on status once the switch input flag is set",
                description="Dry-run generated test case",
                pre_condition="Module initialized, switch input flag cleared",
                setup_action_steps=[ActionStep(verb="SET", detail="switch_input_flag = 1")],
                verification_steps=[
                    VerificationStep(verb="VERIFY", detail="get_light_status() return value", expected_result="LIGHT_ON")
                ],
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="swe6_dry_run_"))
    ...
    ws.append(["1", "When the switch input flag is set, the function shall return light-on status.", "swe6 test"])
    ...
    try:
        with patch("app.core.artifacts.swe6.pipeline.build_chat_model", _fake_build_chat_model), \
             patch("app.core.artifacts.swe6.graph.nodes.call_structured", _fake_call_structured):
            zip_path = generate(dummy_config)
        print(f"\n[swe6 dry run] OK - wrote {zip_path}")
        print(f"[swe6 dry run] intermediate artifacts (enriched_requirements.json, "
