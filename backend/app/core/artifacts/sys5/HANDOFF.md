...
follows one config dict from `generate()` all the way to the finished SYS5 workbook,
...
```python
def generate(config: dict) -> str:  # sys5.py
    # ... returns the path to a .zip containing the finished workbook
```

> **Don't touch.** `sys5.py` has two blocks fenced with `#####Donot change this part in
...
> `generate()` — everything we own sits between those two blocks, in
...
flowchart LR
    ...
    BO -->|"only the .xlsx"| ZIP["zip step<br/>sys5.py — fixed block"]
...
`sys5.py`'s fixed zip step packages *everything* it finds in `output_dir`.
...
`generate()` — can name one of six automotive domains: `bcm`, `adas`, `telematics`,
...
It's injected at a single choke point — `prompts/registry.py::resolve_prompt()` — which
every one of the four agent calls already goes through:

```python
def resolve_prompt(agent_chain, agent_name, domain="") -> str:
...
`python -m app.core.artifacts.sys5.sys5` runs the entire `generate()` flow — Excel
